	...
	function pending_orders_admin_menu(){
		...
		foreach ($latePaymentSubs as $subscription_id) {
		    ...
		    if($pending_order_count<=1){
		    	echo  '<a style="background-color:#D64E07;color:#ffffff;"href="https://vivecul.com.co/wp-admin/post.php?post='.$subscription_id.'.&action=edit" target="_blank">'.$subscription_id."</a>,".$pending_order_count."<br>";
		    }
		    else {
		    	echo  '<a href="https://vivecul.com.co/wp-admin/post.php?post='.$subscription_id.'.&action=edit" target="_blank">'.$subscription_id."</a>,".$pending_order_count."<br>";
		    }
		}
